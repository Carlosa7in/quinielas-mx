    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-6">
        ...
        <div className="space-y-3 print:hidden">
          ...
          {/* ── Enviar directo al cliente por WhatsApp ── */}
          {quiniela.telefonoCliente && (() => {
            const digits  = quiniela.telefonoCliente!.replace(/\D/g, "");
            // Número en formato internacional (México = 52)
            ...
            // Leer folios guardados para incluirlos todos en el mensaje
            let allFolios: string[] = [quiniela.folio];
            try {
              const stored = sessionStorage.getItem("lastRegistro");
              if (stored) allFolios = JSON.parse(stored).folios ?? [quiniela.folio];
            } catch { /* sin sessionStorage */ }

            const ticketUrl = `${typeof window !== "undefined" ? window.location.origin : ""}/ticket/${quiniela.folio}`;

            const foliosTexto = allFolios.length > 1
              ? allFolios.map((f, i) => `*Folio ${i + 1}:* ${f}`).join("\n")
              : `*Folio:* ${quiniela.folio}`;

            const msg = [
              `¡Tu quiniela fue registrada! 🎉`,
              ``,
              foliosTexto,
              `*${quiniela.jornada.liga}* · ${quiniela.jornada.nombre ?? `Jornada ${quiniela.jornada.numero}`}`,
              `*Total:* $${quiniela.monto.toFixed(2)} MXN`,
              ``,
              `🔗 Ver ticket: ${ticketUrl}`,
              ``,
              `Guarda tu folio para reclamar tu premio en caso de ganar 🏆`,
            ].join("\n");

            const href = `https://wa.me/${waPhone}?text=${encodeURIComponent(msg)}`;
            ...
            return (
              ...
              >
                ...
                Enviar a {digits.slice(-10).replace(/(\d{2})(\d{4})(\d{4})/, "$1 $2 $3")}
              </a>
            );
          })()}
          ...
          <button
            ...
            className={`w-full font-bold py-3 rounded-xl transition-colors flex items-center justify-center gap-2 ${
              quiniela.telefonoCliente
                ? "bg-white border-2 border-[#25D366] text-[#25D366] hover:bg-green-50 disabled:bg-gray-100 disabled:border-gray-200 disabled:text-gray-400"
                : "bg-[#25D366] hover:bg-[#20b858] disabled:bg-gray-300 text-white"
            }`}
          >
            {cargandoPNG ? (
            ...
            {cargandoPNG ? "Generando imagen..." : "Compartir imagen del ticket"}
          </button>
