  ...
  useEffect(() => {
    fetch("/api/jornadas")
      ...
      .then((data) => {
        ...
        else setJornada(data);
      })
      .catch(() => setError("Error al cargar jornada"))
  ...
  if (error && !jornada) {
    return (
      <div className="min-h-screen bg-green-900 flex items-center justify-center text-white px-4">
        <div className="text-center">
          <div className="text-4xl mb-4">😕</div>
          <h2 className="text-xl font-bold mb-2">No hay jornada activa</h2>
          <p className="text-green-300">{error}</p>
          <a href="/" className="mt-6 inline-block text-yellow-300 underline">
            Volver al inicio
          </a>
        </div>
      </div>
      ...
      <div className="bg-green-800 text-white py-6 px-4">
        <div className="max-w-lg mx-auto">
          ...
          <p className="text-green-200 text-sm">
            Jornada {jornada?.numero} · {jornada?.temporada} · $20 MXN
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-lg mx-auto px-4 py-6 space-y-6">
        ...
        <div className="space-y-3">
          <h2 className="font-semibold text-gray-700 px-1">
            ...
            <span className="text-green-600 font-normal text-sm">
              ({Object.keys(picks).length}/{jornada?.partidos.length})
            </span>
          </h2>
        ...
        <div className="bg-green-800 text-white rounded-xl p-4">
          <div className="flex justify-between mb-3 text-sm">
            ...
            <span>
              {Object.keys(picks).length}/{jornada?.partidos.length} seleccionados
            </span>
          </div>
