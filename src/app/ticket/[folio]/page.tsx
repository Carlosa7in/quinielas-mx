...
export default function TicketPage() {
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-lg mx-auto px-4 py-6 space-y-5">
          ...
          <button
            ...
            className="w-full text-center text-gray-400 text-sm py-2 hover:text-gray-600 disabled:opacity-50 transition-colors"
          >
            {cambiandoCanal
              ? "Cambiando..."
              : quiniela.canal === "oxxo"
              ? "🔄 Cambiar a transferencia bancaria"
              : "🔄 Cambiar a pago en OXXO"}
          </button>
          ...
          {(() => {
            ...
            return (
              <a
                ...
                className="w-full bg-[#25D366] hover:bg-[#20b858] text-white font-bold py-4 rounded-xl transition-colors flex items-center justify-center gap-2 text-base shadow-md"
              >
          ...
          <p className="text-center text-xs text-gray-400 -mt-2">
            📸 Adjunta la foto de tu comprobante al mensaje de WhatsApp para confirmar más rápido.
          </p>
          ...
          <div className="flex gap-2 pt-1">
            <a
              ...
              className="flex-1 text-center text-amber-700 font-semibold py-2.5 rounded-xl bg-white border border-amber-200 hover:bg-amber-50 transition-colors text-sm"
            >
              🎟️ Otra quiniela
            </a>
            <a
              href={`/consultar?folio=${quiniela.folio}`}
              className="flex-1 text-center text-gray-500 font-semibold py-2.5 rounded-xl bg-white border border-gray-200 hover:bg-gray-50 transition-colors text-sm"
            >
              Ver estado →
            </a>
          </div>
  ...
  return (
    <>
    {/* ── Área de impresión térmica — primero en el DOM para que position:static quede al tope ── */}
    <div id="print-ticket">
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-6">
        ...
        <div className="space-y-3 print:hidden">
          ...
          {esRegistro && quiniela.telefonoCliente && (() => {
            ...
            const waRegular  = `https://wa.me/${waPhone}?text=${encodeURIComponent(msg)}`;
            const waBusiness = isMobile
              ? `intent://send?phone=${waPhone}&text=${encodeURIComponent(msg)}#Intent;scheme=whatsapp;package=com.whatsapp.w4b;end`
              : waRegular;
            const telLabel = tel.slice(-10).replace(/(\d{2})(\d{4})(\d{4})/, "$1 $2 $3");
            const waIcon = (
            ...
            return (
              <div className="space-y-2">
                <p className="text-xs text-gray-400 text-center font-medium">
                  Enviar gracias a {telLabel} con:
                </p>
                <div className="grid grid-cols-2 gap-2">
                  <a href={waRegular} target="_blank" rel="noopener noreferrer"
                    className="bg-[#25D366] hover:bg-[#20b858] text-white font-bold py-3 rounded-xl transition-colors flex items-center justify-center gap-1.5 text-sm">
                    {waIcon} WhatsApp
                  </a>
                  <a href={waBusiness} target="_blank" rel="noopener noreferrer"
                    className="bg-[#128C7E] hover:bg-[#0f7a6d] text-white font-bold py-3 rounded-xl transition-colors flex items-center justify-center gap-1.5 text-sm">
                    {waIcon} Business
                  </a>
                </div>
                {/* Sin WhatsApp — abrir SMS con mensaje pre-llenado */}
                <a
                  href={`sms:+52${waPhone}?body=${encodeURIComponent(msg)}`}
                  className="w-full text-xs text-gray-400 hover:text-gray-600 hover:bg-gray-100 py-2 rounded-xl transition-colors flex items-center justify-center gap-1.5"
                >
                  💬 Sin WhatsApp — enviar SMS
                </a>
              </div>
            );
          })()}
